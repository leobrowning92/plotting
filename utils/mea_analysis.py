...
def analyze_events(smudf, gthresh):
    ...
    iei = np.array(
        [dgthresh[i + 1, 1] - dgthresh[i, 1] for i in range(len(dgthresh) - 1)]
    )
...
def plot_timeslice(tensor, index, ax):
    ...
    cb.set_ticklabels(
        ["{:.2f}".format(cmin + (0.1 * range)), "{:.2f}".format(cmax - (0.1 * range))]
    )
...
def show_timeseries(tensor, start, stop):
    ...
    for i in range(start, stop):
        im = axes.flat[i - start].imshow(
            tensor[i], cmap="viridis", interpolation="nearest"
        )
    for ax in axes.flat:
        ax.axis("off")
...
def plot_loglog(data, ax=None):
    ...
    ax.plot(
        ...
        powerlaw(x, *p),
        "k-",
        label="$\\alpha =${:.2f}$\pm${:.2f}".format(p[0], np.sqrt(dp[0, 0])),
    )
    ax.legend()
...
def plot_events(dg, axes):
    ...
    axes[1].eventplot(
        [event[1] for event in dg if 50 <= event[1] < 60], linewidths=1, alpha=1
    )
    axes[1].set_xlim((50, 60))
    axes[2].eventplot(
        [event[1] for event in dg if 55 <= event[1] < 56], linewidths=1, alpha=1
    )
    axes[2].set_xlim((55, 56))
...
def plot_eventanalysis(smudf, gthresh, name):
    fig = plt.figure()
    axes = []
    spec3 = gridspec.GridSpec(ncols=4, nrows=5, hspace=0.8, wspace=2)
    axes.append(fig.add_subplot(spec3[0, :]))
    axes.append(fig.add_subplot(spec3[1, :]))
    axes.append(fig.add_subplot(spec3[2, :]))
    axes.append(fig.add_subplot(spec3[3:, :2]))
    axes.append(fig.add_subplot(spec3[3:, 2:]))

    dg, iei = analyze_events(smudf, gthresh)
    plot_events(dg, axes[:3])
    plot_loglog(dg[:, 0], axes[3])
    axes[3].set_xlabel("$\Delta G (G_0)$")
    axes[3].set_ylabel("P($\Delta G$)")
    plot_loglog(iei, axes[4])
    axes[4].set_xlabel("$IEI (s)$")
    axes[4].set_ylabel("P($IEI$)")
    for i in range(len(axes)):
        if i in [0, 1, 2]:
            offset = (-0.06, 0.5)
        else:
    ...
    # plt.legend()
    fig.tight_layout()
    plt.savefig(f"plots/{name}eventanalysis.png")
    plt.savefig(f"plots/{name}eventanalysis.pdf")
